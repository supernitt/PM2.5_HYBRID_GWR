...
def main():
    ...
    # Interpolate gaps then Savitzky-Golay smooth every temporal predictor
    # before dropping any rows, so edge gaps are also filled.
    ...
    # Remove entirely-NaN predictors (e.g. elev/pop before static GEE extraction)
    # BEFORE building models — avoids statsmodels endog/exog shape-mismatch error.
    predictors = preprocessing.drop_all_nan_predictors(df, predictors)

    # VIF check (warning only)
    # Pooled VIF is kept for reference but is misleading for ERA5-Land fields:
    # between-station altitude/climate differences drive near-perfect pooled
    # collinearity (VIF up to 6.5M) that is absorbed by the LME random intercepts
    # and is irrelevant to the fixed-effect estimates.
    vif = preprocessing.compute_vif(df, predictors)
    vif.to_csv(output_dir / "vif_pooled.csv", index=False)

    # Within-station VIF: demean each predictor by its per-station mean so that
    # only the temporal within-station variation is assessed — the variation that
    # actually drives LME fixed-effect estimation.  Use this for reporting.
...
# Standardize predictors (fitted on the full df — for CV we'll refit per fold)
    scaler = None
    ...
    # Stage-2 GWR predictor subset (default: AOD only).  Fitting all 12
    # predictors locally is singular in pooled GWR space (ws/t2m VIF in the
    # millions — see vif_pooled.csv), so every day gets discarded by the
    # coefficient sanity check.  Keep only predictors that actually survive
    # preprocessing.  Empty/null in config => fall back to the full set.
    ...
    def build_gwr() -> gwr_model.GWRStage2:
        return gwr_model.GWRStage2(
            ...
            n_jobs=n_jobs,
            bw_cache=_gwr_bw_cache,  # shared: populated on first fit, reused in CV
        )

    def build_hybrid() -> hybrid_model.HybridLMEGWR:
        return hybrid_model.HybridLMEGWR(
            lme=build_lme(), gwr=build_gwr(),
            station_id=cols["station_id"], date_col=cols["date"],
        )

    def build_hybrid_rf(rf_params: dict) -> HybridTreeGWR:
    ...
    def build_hybrid_gb(gb_params: dict) -> HybridTreeGWR:
        return HybridTreeGWR(
    ...
    def build_hybrid_xgb(xgb_params: dict) -> HybridTreeGWR:
        return HybridTreeGWR(
    ...
    # Tuned hyperparameters saved during the train stage so CV folds can reuse
    # them instead of repeating the expensive inner grid-search 20× per model.
    ...
    if "train" in stages:
        ...
        if "hybrid" in algorithms:
            ...
            _lme_only = build_lme().fit(df)
            with open(output_dir / "lme_summary.txt", "w") as f:
                f.write(_lme_only.summary())
    ...
    # ── LME fixed-effect coefficients table ──────────────────────────
    lme_obj = (fitted_hybrid.lme if fitted_hybrid is not None
               else (build_lme().fit(df)
                     if "train" not in stages else None))
    ...
    if "validate" in stages:
        ...
        site_splits = validation.site_kfold_indices(
            stations=df[cols["station_id"]].to_numpy(),
            n_splits=cfg["validation"]["site_kfold"],
            seed=cfg["validation"]["random_seed"],
        )

        all_overall: list[pd.DataFrame] = []
        all_strat: list[pd.DataFrame] = []
        all_cv_preds: list[pd.DataFrame] = []

        # ----- Hybrid LME-GWR -----
        if "hybrid" in algorithms:
            def _train_hybrid(train_df):
                return build_hybrid().fit(train_df)
            ...
            for splits, scheme in [(random_splits, "random_10fold"),
                ...
                o, s, preds = validation.run_cv(
                    df, splits, _train_hybrid, _predict_hybrid,
                ...
                all_overall.append(o); all_strat.append(s); all_cv_preds.append(preds)
        ...
        if "lme" in algorithms:
            ...
            for splits, scheme in [(random_splits, "random_10fold"),
                ...
                _t0 = time.time()
                o, s, preds = validation.run_cv(
                    df, splits, _train_lme, _predict_lme,
        ...
        bench_factories = {}
        if "pcr" in algorithms:
            bench_factories["PCR"] = lambda: benchmarks.PCRModel(n_components=5)
        if "plsr" in algorithms:
            bench_factories["PLSR"] = lambda: benchmarks.PLSRModel(n_components=5)
        if "rf" in algorithms:
            bench_factories["RF"] = lambda: benchmarks.RFModel(n_estimators=500, max_depth=None)
        if "gb" in algorithms:
            bench_factories["GB"] = lambda: benchmarks.GBModel(n_estimators=500, max_depth=3,
                                                               learning_rate=0.05)
        if "xgboost" in algorithms:
            try:
                bench_factories["XGBoost"] = lambda: benchmarks.XGBModel(
                    n_estimators=600, max_depth=6, learning_rate=0.05)
            except ImportError:
                pass

        for name, factory in bench_factories.items():
            def _train(train_df, fct=factory):
                X_tr = train_df[predictors].to_numpy()
                y_tr = train_df[target].to_numpy()
                return fct().fit(X_tr, y_tr)
        ...
        if "hybrid_xgb" in algorithms:
            try:
                benchmarks.XGBModel  # probe import
        ...
        for model_name, grid, tune_fn, builder_fn, pretrained_params in hybrid_tree_specs:
            def _train_htgwr(train_df, _grid=grid, _tune=tune_fn, _build=builder_fn,
                ...
                if _pre is not None:
                    ...
                    params = _tune(X_tr, y_tr, _grid,
                                   n_splits=cfg["validation"]["random_kfold"],
                                   seed=cfg["validation"]["random_seed"])
                return _build(params).fit(train_df)
            ...
            for splits, scheme in [(random_splits, "random_10fold"),
                ...
                _t0 = time.time()
                o, s, preds = validation.run_cv(
                    df, splits, _train_htgwr, _predict_htgwr,
        ...
        strat_df = pd.concat(all_strat, ignore_index=True) if all_strat else pd.DataFrame()
        overall_df.to_csv(output_dir / "cv_overall.csv", index=False)
        if not strat_df.empty:
            strat_df.to_csv(output_dir / "cv_stratified.csv", index=False)
        logger.info("Saved CV metrics:\n%s", overall_df.to_string(index=False))

        # ── CV raw predictions (scatter plot data) ─────────────────────
        cv_preds_df = pd.concat([p for p in all_cv_preds if not p.empty],
                                ignore_index=True)
        cv_preds_df.to_csv(output_dir / "cv_predictions.csv", index=False)
        logger.info("Saved CV raw predictions (%d rows) -> cv_predictions.csv",
                    len(cv_preds_df))

        # ── Per-fold metrics (for error-bar / box plots) ───────────────
        fold_metric_rows = []
        for preds_chunk in all_cv_preds:
            if preds_chunk.empty:
                continue
            for (mdl, scheme, fold), grp in preds_chunk.groupby(
                    ["model", "scheme", "fold"]):
    ...
    if "map" in stages and cfg["mapping"]["enable"]:
        ...
        if grid_df is None:
            ...
            # Standardize grid predictors using full-data scaler
            if cfg["preprocessing"]["standardize_predictors"]:
                grid_df, _ = preprocessing.standardize(grid_df, predictors, fit_on=df)
    ...
    if "map_raster" in stages:
        ...
        # Map from config name → pkl filename and internal model_kind token.
        # hybrid_xgb / hybrid_rf / hybrid_gb use HybridTreeGWR (Stage-1 = tree,
        # Stage-2 = GWR correction) and are recommended over hybrid (LME-GWR)
        # because tree models are immune to the multicollinearity that inflates
        # LME coefficients (t2m VIF=516k, ws VIF=6.5M) and causes extreme
        # predictions at out-of-distribution raster pixels.
        _HYBRID_TREE_PKLS = {
