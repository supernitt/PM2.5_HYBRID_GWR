...
class GWRDailyResult:
    ...
    morans_p_after: float | None = None  # Moran's I p on (residual - correction);
                                         # demonstrates Stage-2 removed the
                                         # spatial autocorrelation it targets
    bw_at_search_floor: bool = False     # True if selected bw == lower search
                                         # bound (flag for sensitivity analysis)
...
class GWRStage2:
    ...
    def __init__(self, predictors: list[str],
        ...
        self.n_jobs = n_jobs
        # Explicit adaptive-bandwidth search range passed to Sel_BW.search.
        # Left as None, mgwr applies its own internal lower bound, which run0
        # showed pins ~70% of days at 45-46 neighbours — an artifact, not an
        # optimum.  Setting bw_min lets AICc actually explore small bandwidths
        # (still clamped at the numerical-stability floor min_stable_bw).
        self.bw_min = bw_min
        self.bw_max = bw_max
    ...
    def fit_day(self, day_df: pd.DataFrame, residuals: np.ndarray,
        ...
        try:
            if fixed_bw is not None:
                ...
                # Explicit search range: without it mgwr's internal default
                # lower bound pinned 70% of run0 days at 45-46 neighbours.
                search_kwargs: dict = {"criterion": self.bw_search}
                if self.adaptive:
                    lo = self.bw_min if self.bw_min is not None else min_stable_bw
                    lo = max(lo, min_stable_bw)          # never below stability floor
                    hi = self.bw_max if self.bw_max is not None else n - 1
                    hi = min(hi, n - 1)
                    if lo < hi:
                        search_kwargs["bw_min"] = lo
                        search_kwargs["bw_max"] = hi
                bw = bw_selector.search(**search_kwargs)
        ...
        # Post-correction Moran's I: the framework's central claim is that
        # Stage-2 removes the residual spatial autocorrelation Stage-1 leaves
        # behind.  Demonstrate it, per day, on the corrected residuals.
        morans_p_after = self._morans_p(residuals - residual_correction, coords)

        # Flag bandwidths sitting on the lower search bound: an "optimum" at
        # the floor is a boundary solution and needs sensitivity analysis.
        _floor = (self.bw_min if self.bw_min is not None else min_stable_bw) \
            if self.adaptive else None
        at_floor = bool(self.adaptive and _floor is not None
                        and int(bw) <= max(_floor, min_stable_bw))

        return GWRDailyResult(date=date, fitted=True,
                              morans_p=morans_p, bandwidth=float(bw),
                              coefficients=coef_df,
                              residual_correction=residual_correction,
                              morans_p_after=morans_p_after,
                              bw_at_search_floor=at_floor)
